...
function populateSpotTable(coins = []) {
	...
	for (const coin of coins) {
		...
		if (totalUsdValue > 20) {
			...
			closeCell.appendChild(sellButton)
		}
	}
}
...
function openSpotCoinModal(coin, quantity, rate, total_usd_value) {
	...
	document.getElementById(
		'modalTotalValue'
	).textContent = `Total USD Value: ${total_usd_value}`
...
function openCloseTradePopup(
	...
	document.getElementById(
		'modalMarginCoin'
	).textContent = `Margin Coin: ${marginCoin}`
	document.getElementById('modalSymbol').textContent = `Symbol: ${symbol}`
	document.getElementById(
		'modalLeverage'
	).textContent = `Leverage: ${leverage}`
	document.getElementById(
		'modalMarginSize'
	).textContent = `Margin Size: ${margin_size}`
	document.getElementById(
		'modalHoldSide'
	).textContent = `Hold Side: ${hold_side}`
...
if (document.querySelector('.confirmSale')) {
	document
		.querySelector('.confirmSale')
		.addEventListener('click', function () {
			confirmAssetsSale('spotTradeModal')
		})
}

if (document.querySelector('.confirmCloseTrade')) {
	document
		.querySelector('.confirmCloseTrade')
		.addEventListener('click', function () {
